...
void SumMatixColsInArray(int MatrixArray[3][3], int arrSum[3], short Rows, short Cols)
{
	for (short i = 0; i < Cols; i++)
	{
		arrSum[i] = ColSum(MatrixArray, Rows, i);
	}

}

void PrintColsSumAraay(int arrSum[3],short length)
{
	for (short i = 0; i < length; i++)
	{
		printf("\n\t col %d sum = %d", i + 1, arrSum[i]);
	}
		cout << endl;
...
int main()
{
	...
	int arrsum[3];
	cout << "\n  the following are the sum of  each col in the matrix from array " << endl;

	SumMatixColsInArray(Array, arrsum, 3, 3);

	PrintColsSumAraay(arrsum, 3);
